...
async function deleteLottery(id){const l=lotteries.find(x=>String(x.id)===String(id));const title=l?.title||'這個一番賞';const ok=confirm(`確定永久刪除「${title}」？\n\n會刪除：\n・這個一番賞本體\n・剩餘籤位與獎項設定\n・原始抽獎紀錄\n\n會永久保留：\n・玩家已抽中的獎品區紀錄\n・獎品名稱、圖片、票號與抽中日期\n・兌換狀態與兌換日期\n・玩家帳號與目前印章\n\n一番賞本體刪除後無法復原。`);if(!ok)return;try{const r=await api('/admin/lotteries/'+id,{method:'DELETE'});toast(`已刪除「${r.title||title}」，保留 ${r.preservedRewards||0} 件玩家獎品`);loadLotteries()}catch(e){toast(e.message)}}
